...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="<?=$_SESSION['translator']->get_language()?>">
  <head>
    ...
    <!--
      $Id: index.tpl,v 1.5.2.2 2004/03/04 20:36:44 letreo Exp $
    -->
    <style type="text/css">
    /*<![CDATA[*/
      .moderator{color: #f00;}
          ...
          <dl>
            ...
            <dd style="padding-left:0;margin-left:0">
              <table>
                <thead>
                  <tr>
                    <td colspan="3" class="success"><?=$TEMPLATE_OUT['success']?></td>
                  </tr>
                  <tr>
                    <td colspan="3" class="error"><?=$TEMPLATE_OUT['error']?></td>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <th>Name:</th>
                    <td>
                      <input <?=$TEMPLATE_OUT['name_disabled']?> name="name" type="text" value="<?=$TEMPLATE_OUT['name']?>" />
                     <?php if($TEMPLATE_OUT['name_disabled']!=''){?>
                      <input name="name" type="hidden" value="<?=$TEMPLATE_OUT['name']?>" />
                     <?php }?>
                    </td>
                    <td rowspan="4" style="padding-left:15px;border-left:1px solid yellow">
                      <select class="inputField" name="selected_channel" style="width:190px;float:left" size="5">
                        <?=$TEMPLATE_OUT['channels_option_list']?>
                      </select>
                      <input style="border:none" type="image" src="<?=$_SESSION['template']->get_theme_path()?>/images/icons/edit_button.gif" name="edit" title="<?=$_SESSION['translator']->out('EDIT')?>" /><br />
                      <input style="border:none" type="image" src="<?=$_SESSION['template']->get_theme_path()?>/images/icons/move_to_top.gif" name="move" title="<?=$_SESSION['translator']->out('MOVE_TO_TOP')?>" /><br />
                      <input style="border:none" type="image" src="<?=$_SESSION['template']->get_theme_path()?>/images/icons/trash.gif" name="delete" title="<?=$_SESSION['translator']->out('DEL_CHANNEL')?>" />
                    </td>
                  </tr>
                  <tr>
                    <th>Password:</th><td><input name="password" type="text" value="<?=$TEMPLATE_OUT['password']?>" /></td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <th>Message:</th><td><input name="message" type="text" value="<?=$TEMPLATE_OUT['message']?>" /></td>
                  </tr>
                  <tr>
                    <th>Type:</th>
                    <td>
                      <select name="type">
                        ...
                        <option value="1" <?=$TEMPLATE_OUT['moderation_selected']?>><?=$_SESSION['translator']->out('MODERATED_CHANNEL')?></option>
                      </select>
                    </td>
                  </tr>
                </tbody>
            ...
            <?php }else{?>
            <input class="submit" style="color:#fff;font-weight:bold;background-color:#0f0;" name="update_channel" type="submit" value="<?=$_SESSION['translator']->out('UPDATE_CHANNEL')?>"/>&nbsp;
            <?php }?>
